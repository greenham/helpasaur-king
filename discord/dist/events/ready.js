...
const WEEKLY_ALERT_MESSAGE = "The weekly Any% NMG Race is starting #startsIn# on <https://racetime.gg> | Create an account (or log in) here: <https://racetime.gg/account/auth> | ALttP races can be found here: <https://racetime.gg/alttp>";
module.exports = {
    name: "ready",
    once: true,
    execute(client) {
        ...
        const weeklyAlertJob = schedule.scheduleJob(timeToSchedule, () => {
            ...
            let alerts = client.config.guilds
                .filter((g) => g.active && g.enableWeeklyRaceAlert && g.weeklyRaceAlertChannelId)
                .map((g) => {
                return {
            ...
            alerts.forEach((a) => {
                ...
                if (!channel)
                    return;
                console.log(`Sending alert to to ${channel.guild.name} (#${channel.name})`);
                let notify = a.roleId ? `<@&${a.roleId}> ` : "";
                let startsIn = `<t:${Math.floor((Date.now() + WEEKLY_ALERT_OFFSET_MINUTES * 60 * 1000) / 1000)}:R>`;
                ...
                channel
                    ...
                    if (channel.guild.id == ALTTP_GUILD_ID) {
                        let ljSmile = channel.guild.emojis.cache.find((emoji) => emoji.name === LJ_SMILE_NAME);
                        channel.send(`<@${REZE_ID}> happy weekly ${ljSmile}`);
                    }
                })
                    .catch(console.error);
            });
            console.log(`Next weekly alert scheduled for: ${weeklyAlertJob.nextInvocation()}`);
        });
        console.log(`Weekly alert scheduled for: ${weeklyAlertJob.nextInvocation()}`);
        ///////////////////////////////////////////////////////////////////////////
        // 2. Rotate activity
        ...
        console.log(`Activity rotation scheduled for: ${activityRotateJob.nextInvocation()}`);
        ///////////////////////////////////////////////////////////////////////////
        // 3. Listen for stream alerts
        wsRelay.on("streamAlert", ({ payload: stream, source }) => {
            if (![STREAM_ONLINE_EVENT, CHANNEL_UPDATE_EVENT].includes(stream.eventType)) {
                return;
            }
            console.log("Received stream alert:", stream.eventType);
            // Get a list of guilds that have stream alerts enabled
            let alerts = client.config.guilds
                .filter((g) => g.active && g.enableStreamAlerts && g.streamAlertsChannelId)
                .map((g) => {
            ...
            alerts.forEach((a) => {
                ...
                if (!channel)
                    return;
                console.log(`Sending stream alert for ${stream.user.login} to ${channel.guild.name} (#${channel.name})`);
                ...
                if (stream.eventType === STREAM_ONLINE_EVENT) {
                    streamAlertEmbed.setImage(stream.thumbnail_url
                        .replace("{width}", 1280)
                        .replace("{height}", 720));
                }
        ...
        wsRelay.on("weeklyRaceRoomCreated", ({ payload: raceData, source }) => {
            ...
            alerts.forEach((a) => {
                ...
                if (!channel)
                    return;
                console.log(`Sending weekly race room alert to ${channel.guild.name} (#${channel.name})`);
                let notify = a.roleId ? `<@&${a.roleId}> ` : "";
                ...
                let weeklyRaceAlertEmbed = new EmbedBuilder()
                    ...
                    .addFields({ name: "Race room", value: raceRoomUrl }, { name: "Goal", value: "Any% NMG" })
                    .setImage("https://racetime.gg/media/The_Legend_of_Zelda__A_Link_to_the_Past-285x380_kyx6ga0.jpg")
                    .setTimestamp()
                    .setFooter({
                    text: source,
                    iconURL: "https://helpasaur.com/img/ljsmile.png",
