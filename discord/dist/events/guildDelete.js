...
module.exports = {
    name: "guildDelete",
    async execute(guild) {
        console.log(`Guild ${guild.name} (${guild.id}) kicked us or the guild was deleted`);
        const { client } = guild;
        // See if there's an existing configuration for this guild
        let guildConfigIndex = client.config.guilds.findIndex((g) => g.id === guild.id);
        ...
        try {
            ...
            console.error(`Error deleting guild ${guild.name} (${guild.id}) via API: ${err.message}`);
        }
    },
};
