    ...
    async execute(interaction) {
        ...
        if (commandNoPrefix === "auth") {
            await interaction.reply({
                content: `https://discord.com/api/oauth2/authorize?client_id=${client.config.clientId}&permissions=${client.config.oauth.permissions}&scope=${client.config.oauth.scopes.join("%20")}`,
                ephemeral: true,
            });
            return;
        }
        ...
        console.log(`Received command <${commandNoPrefix}> from <${author.username}> in guild <${guildConfig.name}>`);
