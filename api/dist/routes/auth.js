...
const { CLIENT_POST_AUTH_REDIRECT_URL, JWT_SECRET_KEY, JWT_HEADER_COOKIE_NAME, JWT_FOOTER_COOKIE_NAME, API_HOST, ALLOWED_SERVICES, } = process.env;
...
router.get(`/twitch`, async (req, res) => {
    ...
    const requestUrl = "https://id.twitch.tv/oauth2/token" +
    ...
    const idToken = jwt.sign({ permissions: localUser.permissions }, JWT_SECRET_KEY, {
        expiresIn: loginExpirationLength,
        subject: localUser._id.toString(),
    });
    console.log(`Generated ID token for Twitch user: ${twitchUserData.display_name}`);
    console.log(idToken);
