...
router.get("/joinUrl", async (req, res) => {
    try {
        ...
        res.status(200).json({
            result: "success",
            message: "OK",
            url: `https://discord.com/api/oauth2/authorize?client_id=${discordConfig.config.clientId}&permissions=${discordConfig.config.oauth.permissions}&scope=${discordConfig.config.oauth.scopes.join("%20")}`,
        });
    }
    catch (err) {
...
// - Creates a new guild
router.post("/guild", requireJwtToken, guard.check(["service"]), async (req, res) => {
    try {
...
router.patch("/guild/:id", requireJwtToken, guard.check(["service"]), async (req, res) => {
    ...
    try {
        ...
        const discordConfig = await Config.findOne({ id: "discord" });
        const index = discordConfig.config.guilds.findIndex((g) => g.id === req.params.id);
        if (index === -1) {
        ...
        discordConfig.config.guilds[index] = Object.assign(discordConfig.config.guilds[index], req.body);
        discordConfig.markModified("config");
        await discordConfig.save();
