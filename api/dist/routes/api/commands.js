...
router.patch("/:id", requireJwtToken, guard.check("admin"), async (req, res) => {
    try {
        const command = await Command.findById(req.params.id);
        if (!command) {
...
router.delete("/:id", requireJwtToken, guard.check("admin"), async (req, res) => {
    try {
...
// services only
router.post("/logs", requireJwtToken, guard.check("service"), async (req, res) => {
    try {
