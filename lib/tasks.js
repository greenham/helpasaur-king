		...
		people.forEach((person) => {
			...
			.then(run => {
				if (run.run && run.run.times && run.run.times.primary_t) {
					console.log(`Found PB of ${run.run.times.primary_t} for ${person.srcUsername}`);
					// update DB
					db.get().collection('tourney-people')
					.update({"_id": db.oid(person._id)}, {$set: {"pb": run.run.times.primary_t}}, (err, res) => {
						if (err) {
							console.error(err);
						} else {
			...
			dClient.on('ready', () => {
				people.forEach((person) => {
					...
					if (foundUser && foundUser.id) {
						// update user
						db.get().collection('tourney-people')
						.update({"_id": db.oid(person._id)}, {$set: {"discordId": foundUser.id}}, (err, res) => {
							if (err) {
								console.error(err);
							} else {
								console.log(`Updated discordId for ${person.discordTag} to ${foundUser.id}`);
							}
						});
					} else {
						console.log(`---> Could not find discord ID for ${person.discordTag}!`)
					}
				});
			})
