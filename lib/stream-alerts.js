...
function StreamAlerts(config) {
    ...
    return new Promise((resolve, reject) => {
      request({ url: requestUrl, method: "POST" }, (err, res, body) => {
        ...
        if (res.statusCode == 200) {
          ...
          reject(
            `Unexpected status code received when fetching access token from Twitch: ${
              res.statusCode
            }`
          );
        }
  ...
  self.newFindStreams = function() {
    return new Promise((resolve, reject) => {
      let searchUrl = `${twitchNewApiBaseUrl}/streams?first=100&game_id=${
        self.config.gameId
      }`;
      ...
      request(search, function(error, response, body) {
        ...
        if (response.statusCode == 200) {
              ...
              err => {
                ...
                if (missingUserData.length > 0) {
                  ...
                  request(search, function(error, response, body) {
                    if (error) {
                      return console.error(
                        `Unable to fetch user data from Twitch for ${
                          missingUserData.length
                        } missing users!`
                      );
                    }
          ...
          reject(
            `Unexpected response from Twitch API when fetching streams! Status: ${
              response.statusCode
            } | Body: ${body}`
          );
        }
