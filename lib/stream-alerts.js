    ...
    return new Promise(async (resolve, reject) => {
      ...
      let chunkResults = chunks.map((chunk) => {
        return new Promise(async (resolve, reject) => {
          ...
          console.log(`Searching chunk of ${chunk.length}`);
          ...
          if (streamSearchData && streamSearchData.length > 0) {
            console.log(
              `Received ${streamSearchData.length} live results, filtering...`
            );
            ...
            console.log(
              `Found ${filteredStreams.length} streams that pass filter`
            );
          } else {
            console.log(`No live results in this slice`);
          }

          // get user data for streams
          // @TODO: need to find a way to await this so resolve doesn't happen early
          if (filteredStreams.length > 0) {
            console.log(
              `Getting user data for ${filteredStreams.length} streams...`
            );
            let userIdsMissingData = [];
            filteredStreams.forEach(async (stream, index) => {
              try {
                let userData = await getCachedUserData(stream.user_id);
                if (userData) {
                  console.log(
                    `Found user data for ${stream.user_id} in cache!`
                  );
                  filteredStreams[index].user = userData;
                } else {
                  userIdsMissingData.push(stream.user_id);
                  console.log(
                    `User data for ${stream.user_id} is NOT cached, added to list to update.`
                  );
                }
              } catch (err) {
                console.error(err);
            ...
            if (userIdsMissingData.length > 0) {
              console.log(
                `Fetching data for ${userIdsMissingData.length} users...`
              );
              ...
              try {
                ...
                usersSearchData.forEach(async (userData) => {
