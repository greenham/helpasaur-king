  ...
  self.watch = () => {
    ...
    client.addListener("message#speedrunslive", (from, message) => {
      if (from === "RaceBot") {
        ...
        let srlUrl = raceChannel
          ? "http://www.speedrunslive.com/race/?id=" + raceChannel[1]
          : null
        let goal = message.match(/\-\s(.+)\s\|/)

        if (
          message.startsWith("Race initiated for " + config.gameName + ". Join")
        ) {
          ...
          message.startsWith("Race finished: " + config.gameName + " - ")
        ) {
          self.emit("done", raceChannel[0], goal[1], srlUrl)
        } else if (
          message.startsWith("Rematch initiated: " + config.gameName + " - ")
        ) {
