...
const indexedCategories = readCategories(
  path.join(__dirname, "..", "conf", "src_categories")
)
...
function findWR(game, majorCat, minorCat) {
  return new Promise((resolve, reject) => {
    ...
    if (!subcategory) {
      resolve(
        "Not a valid sub-category name! Codes are listed here: https://github.com/greenham/alttp-bot/blob/master/README.md#category-codes"
      )
    }
        ...
        request(wrSearchReq, function (error, response, body) {
          if (!error && response.statusCode == 200) {
            ...
            if (data && data.data && data.data.runs) {
              ...
              let wrResponse =
                `The current world record for *${category.name} | ${subcategory.name}` +
            ...
            resolve(
              `No record found for *${category.name} | ${subcategory.name}*`
            )
          } else {
...
function findRun(data, category, subcategory) {
  if (data && data.data) {
    ...
    let run = data.data.find(function (r) {
      return (
        r.run.category === category.id &&
        r.run.values[subcategory.varId] === subcategory.id
      )
    })
