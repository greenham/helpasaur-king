...
// @TODO: Read from a passed-in configuration, don't assume outside config
...
function login() {
  return new Promise((resolve, reject) => {
    if (!config.srtv || !config.srtv.username || !config.srtv.password) {
      reject("SRTV has not been configured properly! Check config!")
    }
        ...
        if (value == undefined) {
          ...
          apiRequest(req, function (error, response, body) {
            if (error) {
              ...
              reject(
                `Unexpected response from SRTV during login: ${JSON.stringify(response)}`
              )
            }
          })
        } else {
  ...
  return new Promise((resolve, reject) => {
    ...
    apiRequest(req, function (error, response, body) {
      if (error) {
        ...
        reject(
          `Unexpected response from SRTV during getRace: ${JSON.stringify(response)}`
        )
      }
    })
  })
      ...
      .then((token) => {
        ...
        apiRequest(req, function (error, response, body) {
          if (error) {
            ...
            reject(
              `400 Error from SRTV: ${response.headers.hydra - error - code}: ${
                response.headers.hydra - error - text
              }`
            )
          } else if (response.statusCode == 403) {
            reject(
              `403 Error from SRTV: No user provided or user does not have permission to create a race.`
            )
          } else {
            reject(
              `Unexpected response from SRTV during createRace: ${JSON.stringify(response)}`
            )
          }
        })
      })
    ...
    return new Promise((resolve, reject) => {
      const start = async () => {
        await util.asyncForEach(message, async (msg) => {
          await sendChatMessage(guid, msg).catch((err) => {
            reject(err)
          })
        })
        resolve(true)
      ...
      .then((token) => {
        ...
        apiRequest(req, function (error, response, body) {
          if (error) {
            ...
            reject(
              `Unexpected response from SRTV during sendChatMessage: ${JSON.stringify(response)}`
            )
          }
        })
      })
...
function getChatHistory(guid) {
  return new Promise((resolve, reject) => {
    let req = { url: `${baseUrl}/chats/${guid}` }
    ...
    apiRequest(req, function (error, response, body) {
      if (error) {
        ...
        reject(
          `Unexpected response from SRTV during getChatHistory: ${JSON.stringify(response)}`
        )
      }
    })
  })
...
function getRaceEntries(guid) {
  return new Promise((resolve, reject) => {
    let req = { url: `${baseUrl}/races/${guid}/entries` }
    ...
    apiRequest(req, function (error, response, body) {
      if (error) {
        ...
        reject(
          `Unexpected response from SRTV during getRaceEntries: ${JSON.stringify(response)}`
        )
      }
    })
  })
