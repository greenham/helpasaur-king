...
function upcoming(event, interval, cb) {
  return new Promise((resolve, reject) => {
    ...
    let start = now.subtract({ hours: 1 }).format()
  ...
  return new Promise((resolve, reject) => {
    ...
    request(req, function (error, response, body) {
      if (error) {
        reject(
          new Error(
            `${response.statusCode} Error Response from SpeedGaming API: ${error}`
          )
        )
        ...
        if (data) {
          ...
          reject(
            new Error(
              "Unexpected response/format from SpeedGaming API: " + data
            )
          )
        }
        ...
        reject(
          `Unexpected response from SpeedGaming during getEventSchedule: ${JSON.stringify(response)}`
        )
      }
    })
  })
