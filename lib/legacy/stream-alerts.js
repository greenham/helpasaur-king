    ...
    cache.get(cacheKey, function (err, userData) {
      ...
      if (userData !== null) {
        ...
        try {
          ...
          return reject(`Error parsing JSON from cached user data: ${err}`)
        }
      } else {
        // cache miss
        resolve(false)
        ...
        return new Promise(async (resolve, reject) => {
          ...
          if (filteredStreams.length > 0) {
            ...
            if (userIdsMissingData.length > 0) {
              ...
              let usersSearchParams = `/users?${userIdsMissingData.map((x) => `id=${x}`).join("&")}`

              try {
                let usersSearchResponse =
                  await axiosInstance.get(usersSearchParams)
